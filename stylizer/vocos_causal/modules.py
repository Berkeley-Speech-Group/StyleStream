...
class ConvNeXtBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond_embedding_id: Optional[torch.Tensor] = None) -> torch.Tensor:
        residual = x
        if self.causal:
            x = self.dwconv(F.pad(x, pad=(6, 0), mode='constant', value=0))
        else:
            x = self.dwconv(x)
    ...
    def realtime_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.left_context is None:
            self.left_context = torch.zeros(x.size(0), x.size(1), 6, device=x.device)  # [B, C, 6]
        
        x_cat = torch.cat([self.left_context, x], dim=2)  # [B, C, 6 + t_chunk]
        x_out = self.dwconv(x_cat)  # [B, C, t_chunk]
        self.left_context = x[:, :, -6:]  # update left context
        
        x_out = x_out.transpose(1, 2)  # (B, C, T) -> (B, T, C)
        ...
        x_out = x_out.transpose(1, 2)  # (B, T, C) -> (B, C, T)
